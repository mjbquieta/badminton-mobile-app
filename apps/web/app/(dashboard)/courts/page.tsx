...
export default function CourtsPage() {
  ...
  return (
    <div className="p-4 sm:p-6 md:p-8 max-w-5xl">
      ...
      {/* Add Court Modal */}
      <Modal open={showAddModal} onClose={() => { setShowAddModal(false); setAddCount(1); }} title="Add Courts">
        <div className="space-y-4">
          ...
          <div>
            <label className="text-sm text-light-200 mb-2 block">How many?</label>
            <div className="flex items-center gap-3">
              <button
                onClick={() => setAddCount((c) => Math.max(1, c - 1))}
                disabled={addCount <= 1}
                className="w-10 h-10 rounded-xl bg-dark-200 text-light-100 font-bold text-lg hover:bg-dark-100 disabled:opacity-30 disabled:cursor-not-allowed"
              >
                &minus;
              </button>
              <span className="text-2xl font-bold w-10 text-center tabular-nums">{addCount}</span>
              <button
                onClick={() => setAddCount((c) => c + 1)}
                className="w-10 h-10 rounded-xl bg-dark-200 text-light-100 font-bold text-lg hover:bg-dark-100"
              >
                +
              </button>
            </div>
          ...
          <div className="flex gap-3 justify-end pt-2">
            <button onClick={() => { setShowAddModal(false); setAddCount(1); }} className="px-4 py-2 rounded-xl text-light-200 hover:bg-dark-200">
              Cancel
            </button>
            <button onClick={handleAddCourts} className="px-4 py-2 rounded-xl bg-accent text-primary font-semibold hover:bg-accent/80">
              Add {addCount} {addCount === 1 ? 'Court' : 'Courts'}
            </button>
          </div>
        </div>
      </Modal>
